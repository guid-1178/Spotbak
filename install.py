...
class spotify(object):
    def __init__(self, config):
        ...
        self.client_encoded = base64.b64encode(
            bytes(self.client_id + ":" + self.client_secret, "utf-8")
        )
    ...
    def get_token(self, code):
        headers = {
            ...
            "Authorization": "Basic  {0}".format(self.client_encoded.decode("utf-8")),
        }
        payload = {
        ...
        r = requests.post(
            self.account_url + "/api/token", headers=headers, params=payload
        )
        return r.text
    ...
    def get_playlist(self, access_token, userid):
        headers = {"Authorization": "Bearer  {0}".format(access_token)}
        r = requests.get(
            self.api_url + "/v1/users/{0}/playlists".format(userid), headers=headers
        )
        return json.loads(r.text)
    ...
    def get_tracks(self, access_token, plid):
        headers = {"Authorization": "Bearer  {0}".format(access_token)}
        r = requests.get(
            self.api_url + "/v1/playlists/{0}/tracks".format(plid), headers=headers
        )
        return json.loads(r.text)
...
def installdb(dbPath, dbFullPath, csvPath):
    ...
    tablecheck = db.query(
        """SELECT name FROM sqlite_master WHERE type='table' AND name='users_spotbak';"""
    ).fetchone()
...
def UserCheck(user_info, dbFullPath):
    ...
    result = db.query(
        """ SELECT userid from users_spotbak where userid= '{0}';""".format(
            user_info["id"]
        )
    ).fetchone()
    if result:
