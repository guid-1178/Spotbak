...
import  os, sys, getopt, csv, time, re, yaml
...
def menu(argv):
	...
	for opt, arg in opts:
		if opt == '-h':
			...
			print('-u	--user=		set user (note: include commas with name, ie: "John Smith")')
			sys.exit()
		elif opt in ("-l", "--list"):
...
def write2csv(csvfile,track,album,artist,track_id,snapshot):
	try:
		...
		with open(csvfile, 'w') as csv_file:
			track_writer = csv.writer(csv_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
			track_writer.writerow(['Track','Album','Artist','Track_id','Snapshot_at'])
...
def main():
	...
	if "error" in response:
		if response['error']['status'] == 401:
			...
			db.query(''' UPDATE users_spotbak SET 'access_token' = '{0}', last_updated= CURRENT_TIMESTAMP WHERE user = '{1}';'''.format(token['access_token'],user))
